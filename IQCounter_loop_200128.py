...
Can read input station from commandline using:
    >> nohup python -u ./IQCounter_loop_200128.py > BBWL.log &    
...
#%% Settings that define the seismic data that we'll look at.
...
pp = dict()
pp['coarse_duration'] = 10*60 # s  How many seconds of seismic data to assess at a time.
pp['coarse_overlap'] = 0.5 # []  Ratio of time to overlap between successive coarse_duration times. Often 50% overlap.
pre_filt = (0.1, .2, 230, 250.)
...
# Saving the objects:
with open('Events_' + station + '_200128.pickle', 'wb') as f:  # Python 3: open(..., 'wb')
